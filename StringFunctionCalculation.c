...
size_t cnt[MAX_LEN + 3], s[MAX_LEN + 3], sa[MAX_LEN + 3], r[MAX_LEN], h[MAX_LEN + 1], S[MAX_LEN + 1];
...
int main(int argc, char *argv[]) {
	...
	size_t n, len, top;
	unsigned long long alt, ans;
	scanf("%s", str);
	for (max_val = 0, n = 0; str[n]; ++n) { s[n] = str[n]; if (str[n] > max_val) max_val = str[n]; }
	len = n;
	for (n = len + 1; n < len + 3; ++n) sa[n] = s[n] = 0;
	compute_suffix_array(s, sa, len + 1, max_val);
	compute_lcp_array(len, sa, r, h);
	for (ans = 0, top = 0, n = 1; n <= len + 1; ) if (!top || h[n] >= h[S[top - 1]]) S[top++] = n++; else if (--top, (alt = 1ULL * h[S[top]] * (top ? n - S[top - 1] : n + 1)) > ans) ans = alt;
	printf("%llu\n", ans > len ? ans : len);
	return 0;
}
